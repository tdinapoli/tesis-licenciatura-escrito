...
La relación entre la desviación estandar $\sigma$ de una distribución gaussiana, \todo{esto está mal:} y la desviación estandar $\sigma_c$ su autocorrelación está dada por $\sigma = \sigma_c/\sqrt{2}$.
...
Las mediciones que mostramos en esta sección fueron útiles para definir determinar que un pulso de la señal es un fotón sólo si su máximo es menor a 1 V.
...
Para esto, se necesita un algoritmo que tome como entrada la señal temporal del PMT y devuelva como salida el conjunto de tiempos $\{t_p\}$ en los que hay un pulso.
...
Nuestra implementación, que llamaremos DB, consiste en tres paso, (i) binariza la señal temporal, (ii) busca los puntos en los que la derivada hace un salto y (iii) elimina los puntos que están muy cerca en el tiempo.
...
Luego, se calcula la diferencia entre los puntos consecutivos $\Delta t_p$ y se toma el conjunto $\{t_b\}$ en los que la diferencia es negativa.
...
Para eso iluminamos una muestra con intensidad constante durante un intervalo $T$ de medición, calculamos la distribución de $\Delta t_p$ y la comparamos con una simulación equivalente (\textbf{Fig. \ref{fig:conteo}}).
La diferencia $\Delta t_p = t_{p+1} - t_p$ entre dos variables aleatorias uniformemente distribuidas, y ordenadas de forma tal que $t_1 < ... < t_p < t_{p+1} < ... < t_N $ es otra variable aleatoria con distribución exponencial y valor medio $T/N$, donde $N$ es el número de pulsos detectados.
La simulación consiste en N = 30000 pulsos uniformemente distribuidos en un intervalo de $\sim 96$ ms, lo que da un valor medio de $\lambda \equiv \langle \Delta t \rangle = 3.22$ $\mu$s (\textbf{Fig. \ref{fig:conteo}A y C}).
...
El algoritmo \textit{DB} detectó 3218 pulsos, y se ve que la distribución de $\Delta t$ parece seguir una exponencial excepto por el primer intervalo del histograma que tiene un número de cuentas mucho mayor que el resto (\textbf{Fig. \ref{fig:conteo}E y G}).
Esto se debe a las fluctuaciones de alta frecuencia, que hacen que \textit{DB} detecte dos $t_p$ para un mismo pulso de la señal. 
...
Obtuvimos los estadísticos de la tabla \ref{tab:chisq},
...
\noindent donde el $\chi^2$ crítico es el correspondiente a un test de hipótesis con significancia $\alpha = 0.05$.
